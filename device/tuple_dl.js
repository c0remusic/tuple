...
function dlWin(url, shaUrl) {
    ...
    downloadVerified(url, dest, shaUrl, function(err) {
        ...
        // Un échec de LANCEMENT (ENOENT, blocage antivirus) émet 'error' : sans listener,
        // l'exception tue le process node.script et _busy reste true — l'updater serait
        // mort en silence jusqu'au redémarrage. fail() rend la main à l'UI.
        proc.on('error', function(err) { maxApi.post('tuple_dl: spawn error: ' + err.message); fail('spawn'); });
        proc.unref();
