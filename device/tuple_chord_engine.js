...
function key(k) {
	...
	if (NOTE_TO_PC[k] !== undefined) {
		_releaseHeld();   // #011 : changer la tonalité sous une note tenue la laissait sonner
		root = NOTE_TO_PC[k];
		pushUIState();
	}
}
...
// #011 cause 2 : `root = parseInt(v)` posait NaN pour une valeur non numérique, et NaN
// traversait ensuite les calculs de la grille — broadcastGrid() levait, flatGrid restait
// VIDE, et midinote() sortait tôt pour TOUTE touche. Le clavier mourait pendant que les
// clics souris continuaient de marcher, exactement l'asymétrie signalée.
function rootidx(v) {
	var i = parseInt(v);
...
function setscale(s) {
	...
	if (SCALES[s]) {
		_releaseHeld();   // #011 : changer de gamme sous une note tenue la laissait sonner
		scale = SCALES[s];
		scaleName = s;
		_vl2_reset();
	}
...
// Même chaîne de panne que rootidx : un NaN ici se propage jusqu'au registre et vide la
// grille. BORNER plutôt qu'ignorer — un clic molette en butée doit rester en butée, pas
// être rejeté. (#011, rétroporté de main.)
...
function voicing(v) {
	_releaseHeld();      // #011 : changer de voicing sous une note tenue la laissait sonner
...
// Le `|| "classic"` d'origine masquait déjà l'index invalide, mais SANS libérer la note
// tenue : le voicing changeait sous les doigts et l'accord continuait de sonner (#011).
function voicingidx(v) {
	var i = parseInt(v);
	if (isNaN(i) || i < 0 || i >= VOICING_NAMES.length) { post("voicingidx: index hors énumération ignoré (" + v + ")\n"); return; }
	_releaseHeld();
	currentVoicing = VOICING_NAMES[i];
	_vl2_reset();
...
// Libère la note tenue ET réarme activeMidiNote. Les deux DOIVENT bouger ensemble : sans le
// second, le dédoublonnage de midinote() (pitch === activeMidiNote) avale silencieusement le
// prochain appui de la même touche — la « touche morte ». Deux globals qui portent un seul
// état, d'où cette fonction plutôt que la paire recopiée sur chaque site.
// Rétroporté de main pour le bug #011 (v1.4.2).
